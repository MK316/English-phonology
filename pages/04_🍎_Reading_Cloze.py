import streamlit as st
import streamlit.components.v1 as components
from pathlib import Path

# ---------------- Page setup ----------------
st.set_page_config(
    page_title="AEP Reading & Cloze Quiz",
    layout="wide"
)

st.markdown("### Chapters")

# ---------------- File path ----------------
BASE_DIR = Path(__file__).resolve().parent
EXERCISE_DIR = BASE_DIR / "readings"

# ---------------- Chapter menu ----------------
chapters = {
    "Chapter 1A": "reading_cloze01.html",
    "Chapter 1B": "reading_cloze01.html",
    "Chapter 1C": "reading_cloze01.html",
    "Chapter 2": "ex02_test.html",
    "Chapter 3": "ex03_test.html",
    "Chapter 4": "ex04_test.html",
    "Chapter 5": "ex05_test.html",
}

selected_chapter = st.selectbox(
    "Select a chapter",
    options=list(chapters.keys())
)

# ---------------- Load selected HTML ----------------
html_file = EXERCISE_DIR / chapters[selected_chapter]

if html_file.exists():

    html = html_file.read_text(
        encoding="utf-8"
    )

    components.html(
        html,
        height=1100,
        scrolling=True
    )

else:
    st.error(
        f"File not found: {html_file.name}"
    )
